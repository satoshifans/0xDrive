...
import Batch "./ERC1155/Batch";

actor class FileManager(init_admin: Principal) = this{
  ...
  type BatchError = Batch.BatchError;
  ...
  private var _registry = HashMap.HashMap<TokenIndex, TokenLedger>(1, Text.equal, func(x : Text) : Hash.Hash {Text.hash(x)});
  ...
  private var _metadata = HashMap.HashMap<TokenIndex, (Metadata, Balance)>(1, Text.equal, func(x : Text) : Hash.Hash {Text.hash(x)});
  ...
  public shared(msg) func registerToken(request: RegisterTokenRequest) : async Result.Result<TokenIndex,Text> {
    ...
    let tokenId = switch(request.metadata){
      case(#Fungible fungibleMetadata) {
        fungibleMetadata.dataId;
      };
      case(#Nonfungible nonfungibleMetadata){
        nonfungibleMetadata.dataId;
      };
    };
  ...
  public shared(msg) func transfer(request: TransferRequest) : async TransferResponse {
    await _doTransfer(msg.caller, request);
  };
  ...
  public shared(msg) func transfer_batch(request : [TransferRequest]) : async Result.Result<[TransferResponse], BatchError> {

    var responseArray : [TransferResponse] = [];

    for (req in request.vals()) {
       switch (await _doTransfer(msg.caller, req)){
        case(#ok(balance)) {
          responseArray := Array.append(
            responseArray,
            Array.make<TransferResponse>(
              #ok(balance)
            )
          );
        };
  ...
  public query func supply(token : TokenIdentifier) : async Result.Result<Balance, CommonError> {
    let tokenIndex = ExtCore.TokenIdentifier.getIndex(token);
    Debug.print(debug_show(tokenIndex));
  ...
  func _doTransfer(caller : Principal, request: TransferRequest) : async TransferResponse {
    let tokenIndex = ExtCore.TokenIdentifier.getIndex(request.token);
    var tokenBalances = switch(_registry.get(tokenIndex)) {
      case (?balances) balances;
      case (_) return #err(#InvalidToken(request.token));
